...
async def rsi_test():
    async def strategy(ctx):
        ...
        if run_data["entries"] is None:
            ...
            times = await alea.Time(ctx, max_history=True, use_close_time=True)
            ...
            await alea.plot_indicator(ctx, "RSI", times[delta:], rsi_v, run_data["entries"])
        if alea.current_live_time(ctx) in run_data["entries"]:
            ...
            await alea.market(ctx, "buy", amount="10%", stop_loss_offset="-15%", take_profit_offset="25%")
    ...
    res = await alea.run(data, strategy, config)
    print(res.describe())
...
# Alea-Bot runs using the python asyncio framework.
