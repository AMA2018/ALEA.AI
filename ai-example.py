...
async def basic_evaluation_function(ctx):
    closes = await alea.Close(ctx, max_history=True)
    open = await alea.Open(ctx, limit=30)
    high = await alea.High(ctx, limit=30)
    low = await alea.Low(ctx, limit=30)
    vol = await alea.Volume(ctx, limit=30)
...
async def run_strategy(data, env, agent, symbol, time_frame, is_training=False, plot=False):
    ...
    res = await alea.run(data, strategy, {}, enable_logs=False)
...
def main():
    ...
    data = asyncio.run(alea.get_data(symbol.merged_str_symbol(), time_frame, exchange=args.exchange, start_timestamp=int(float(str((datetime.now() - timedelta(days=args.days)).timestamp()))))) # start_timestamp=1505606400
    ...
    agent = alea.DQNAgent(action_size)
